...
1. カスタ厶Makefileの指定
	...
	```
	・画面左側「YsTitle」プロジェクトを右クリックし、「Properties…」を選択する。
...
1. 「main.c」の作成
	...
	・３番目の画面
		...
		保存場所「C:¥sgdk151¥projects¥YsTitle」を確認する。
		「src」フォルダを作成。
...
1. 「resource.res」の作成
	...
	```
	・「File → New → File… → Empty file」を選択し「Go」ボタンを押す。
	・１番目の画面「Next >」を押す。
	・２番目の画面
		「Filename with full path」の「…」ボタンを押す。
		保存場所「C:¥sgdk151¥projects¥YsTitle」を確認する。
		「res」フォルダを作成。
		「res」フォルダに入って下の「ファイル名」欄に、
			ファイル名 → resource.res
		と入力して「保存」ボタンを押す。
